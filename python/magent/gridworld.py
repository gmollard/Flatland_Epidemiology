...
class GridWorld(Environment):
    ...
    def epidemy_contained(self):
        buf = np.empty(1, dtype=np.int32)
        _LIB.gridworld_epidemy_contained(self.game, buf.ctypes.data_as(ctypes.POINTER(ctypes.c_int32)))
        return buf
...
class Config:
    ...
    def __init__(self):
        ...
        self.groups = []
        self.groups_init_infected_prop = []
        self.reward_rules = []
    ...
    def set_infection_mode(self):
        self.infection_mode = True
    ...
    def add_group(self, agent_type, prop_infected=0):
        ...
        if self.infection_mode:
            self.groups_init_infected_prop.append(prop_infected)
        return no
