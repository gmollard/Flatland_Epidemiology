...
void GridWorld::new_group_infected(const char* agent_name, GroupHandle *group, float prop_infected) {
    *group = (GroupHandle)groups.size();
    ...
    std::cout << "new group infected" << std::endl;

    groups.push_back(Group(iter->second, prop_infected));
}
...
void GridWorld::set_infection_mode() {
    infection_mode = true;
}
...
float GridWorld::get_prop_infected_init(GroupHandle group) const {
    return groups[group].get_prop_infected_init();
}
