...
class Group {
public:
    Group(AgentType &type) : type(type), prop_infected_init(0.0), dead_ct(0), next_reward(0),
                             center_x(0), center_y(0), recursive_base(0) {
    }

    Group(AgentType &type, float prop_infected) : type(type), prop_infected_init(prop_infected), dead_ct(0), next_reward(0),
                             center_x(0), center_y(0), recursive_base(0) {
        std::cout << "Initial Prop of infected agents :" <<  prop_infected << std::endl;
    }
    ...
    float get_prop_infected_init() const{ return prop_infected_init; }
...
private:
    ...
    // Proportion of the group infected at t = 0.
    float prop_infected_init;
};
