...
class AgentType {
public:
    AgentType(int n, std::string name, const char **keys, float *values, bool turn_mode, bool infection_mode);
    ...
    float vaccine_radius, vaccine_angle;
    ...
    int vacc_x_offset,  vacc_y_offset;
    ...
    Range *view_range, *attack_range, *move_range, *vaccine_range;

    int move_base, turn_base, attack_base, vaccine_base;
    std::vector<int> action_space;


    // Infection radius
    float infection_radius;
    Range *infection_range;
